...
public class WorkPackageRepository : IWorkPackageRepository
{
    ...
    public async Task ResetSignatories()
    {
        if (!TryGetApplicationId(out var applicationId))
        {
            return;
        ...
        await _connection.ExecuteAsync("ResetWorkPackageSignatories", new
        {
            ApplicationId = applicationId
        });
    ...
    public async Task InsertSignatories(bool? areSignatoriesCorrect)
    {
        if (!TryGetApplicationId(out var applicationId))
        {
            return;
        ...
        {
            ApplicationId = applicationId,
            AreSignatoriesCorrect = areSignatoriesCorrect
        });

        scope.Complete();
    }

    public async Task UpdateSignatories(bool? areSignatoriesCorrect)
    {
        if (!TryGetApplicationId(out var applicationId))
        {
            return;
        ...
        await _connection.ExecuteAsync("UpdateWorkPackageSignatories", new
        {
            ApplicationId = applicationId,
            AreSignatoriesCorrect = areSignatoriesCorrect
        });
    ...
    public async Task<SignatoriesResult> GetSignatories()
    {
        if (!TryGetApplicationId(out var applicationId))
        {
            return null;
        ...
        var results = await _connection.QueryAsync<SignatoriesResult, Signatory, SignatoriesResult>(
            "GetWorkPackageSignatories",
            (signatoriesResult, signatory) =>
            {
                if (!string.IsNullOrEmpty(signatory.Name))
                {
                    signatories.Add(signatory.Name);
                }

                return signatoriesResult;
            },
            new
            {
                ApplicationId = applicationId
        ...
        {
            uniqueResult.Signatories = signatories;
        }

        return uniqueResult;
    }

    public async Task<ETaskStatus?> GetSignatoriesStatus()
    {
        if (!TryGetApplicationId(out var applicationId))
        {
            return null;
        ...
        return await _connection.QuerySingleOrDefaultAsync<ETaskStatus?>("GetWorkPackageSignatoriesStatus", new
        {
            ApplicationId = applicationId
        });
    }

    public async Task UpdateSignatoriesStatus(ETaskStatus taskStatus)
    {
        if (!TryGetApplicationId(out var applicationId))
        {
            return;
        ...
        await _connection.ExecuteAsync("UpdateWorkPackageSignatoriesStatus", new
        {
            ApplicationId = applicationId,
            TaskStatusId = taskStatus
        });
    ...
    public async Task UpdateSignatoriesContactTask(bool contactTaskRaised)
    {
        if (!TryGetApplicationId(out var applicationId))
        {
            return;
        ...
        await _connection.ExecuteAsync("UpdateWorkPackageSignatoriesContactTask", new
        {
            ApplicationId = applicationId,
            ContactTaskRaised = contactTaskRaised
        });
