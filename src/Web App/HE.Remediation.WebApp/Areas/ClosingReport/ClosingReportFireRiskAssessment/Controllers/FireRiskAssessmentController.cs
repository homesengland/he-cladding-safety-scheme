...
using HE.Remediation.Core.UseCase.Areas.ClosingReport.SetFraewEvidence;
...
public class FireRiskAssessmentController(ISender sender, IMapper mapper) : BaseAboutController(sender)
{
    ...
    {
        var response = await _sender.Send(new GetUploadRequest(EClosingReportFileType.ExitFraew));
    ...
    {
        var validator = new FraewUploadViewModelValidator();
        ...
        {
            await _sender.Send(new SetFraewEvidenceRequest(model.ExitFraewDocumentType));
        }
